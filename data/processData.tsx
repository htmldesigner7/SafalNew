import { ReactNode } from 'react';

export interface ProcessItem {
  id: number;
  title: string;
  description: ReactNode;
}

export const aboutProcessData = [
  {
    id: 0,
    title: "01. Strategic Discovery",
    description: "Define direction through clarity and system-level understanding."
  },
  {
    id: 1,
    title: "02. Low-Code Engineering",
    description: "Accelerate development with scalable, modular architecture."
  },
  {
    id: 2,
    title: "03. Frictionless Integration",
    description: "Connect systems seamlessly across platforms and environments."
  },
  {
    id: 3,
    title: "04. Autonomous Scaling",
    description: "Enable systems to evolve, adapt, and scale independently."
  }
];

export const startupSupportData = [
  {
    id: 0,
    title: "01. Tech Consulting",
    description: "Provides startup consulting services focused on technology selection, system feasibility, and long-term scalability."
  },
  {
    id: 1,
    title: "02. Architecture Design",
    description: "Designs scalable system architecture aligned with product growth, performance requirements, and integration needs."
  },
  {
    id: 2,
    title: "03. Product Roadmapping",
    description: "Defines structured development roadmaps with clear phases, feature prioritization, and execution timelines as part of startup consultancy services."
  }
];

export const specializedSystemsData = [
  {
    id: 0,
    title: "01. Distributor Management System (DMS)",
    description: "Manages distributor networks, order workflows, and inventory movement with structured visibility across channels."
  },
  {
    id: 1,
    title: "02. Tenant Management System (TMS)",
    description: "Structures property and tenant lifecycle management, including billing, records, and operational workflows within a controlled system environment."
  },
  {
    id: 2,
    title: "03. Document Management System",
    description: "Centralizes document control, access governance, and workflow standardization to support compliance and operational consistency."
  },
  {
    id: 3,
    title: "04. TaskNote (Task Management)",
    description: "Structures task allocation, tracking, and execution across teams, ensuring accountability and visibility across operational processes."
  }
];

export const engineeringCapabilitiesData = [
  { id: 0, title: "01. Design & Engineering", description: "System design aligned with plant operations and scalability." },
  { id: 1, title: "02. PLC / SCADA Systems", description: "Control systems for monitoring and centralized visibility." },
  { id: 2, title: "03. IIoT Integration", description: "Seamless iiot integration for real-time data connectivity." },
  { id: 3, title: "04. Smart Factory Solutions", description: "Deployment of smart factory solutions and smart manufacturing solutions for connected production environments." }
];

export const enterpriseAiCapabilitiesData = [
  {
    id: 0,
    title: "01. AI Advisory",
    description: "Structured AI advisory services focused on defining strategy, identifying use cases, and aligning AI initiatives with business systems."
  },
  {
    id: 1,
    title: "02. AI Product Development",
    description: "End-to-end AI product development services delivered by an experienced AI product development company, focused on scalable and usable AI systems."
  },
  {
    id: 2,
    title: "03. AI Implementation",
    description: "Deployment of AI systems through structured AI implementation services, ensuring integration with enterprise workflows and platforms."
  },
  {
    id: 3,
    title: "04. Agentic Systems",
    description: "Design and development of autonomous systems, including developing an agentic AI system for task execution, workflow automation, and decision support."
  }
];


export const startupSupportServices = [
  {
    id: 0,
    title: "01. Bookkeeping",
    description: "Structured bookkeeping services ensuring accurate recording and maintenance of financial transactions."
  },
  {
    id: 1,
    title: "02. Tax Preparation",
    description: "Delivery of tax preparation services aligned with applicable tax regulations and reporting requirements."
  },
  {
    id: 2,
    title: "03. CPA Support",
    description: "Dedicated CPA support services providing structured accounting support, documentation, and reporting assistance."
  }
 
];

export const structuredExecutionData = [
  {
    id: 0,
    title: "01. Strategy",
    description: "We begin by understanding business objectives, operational challenges, and growth priorities. This foundation shapes a structured roadmap, ensuring every system and solution we build aligns with long-term enterprise outcomes.",
  },
  {
    id: 1,
    title: "02. Design",
    description: "We translate strategy into structured system architecture and user experience frameworks, spanning ERP interfaces, digital platforms, and brand touchpoints. This ensures consistency and scalability as the business evolves.",
  },
  {
    id: 2,
    title: "03. Execution",
    description: "We bring strategy and design to life through structured implementation, integrating enterprise systems, automation, and digital platforms. Every deliverable is aligned with defined timelines and technical precision from planning through deployment.",
  },
  {
    id: 3,
    title: "04. Optimization",
   description: "We continuously monitor system performance, analyze operational data, and refine processes through ongoing optimization. This ensures sustained efficiency, scalability, and alignment between technology and business outcomes over time.",
  }
];
